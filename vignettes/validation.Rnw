...
For this purpose, the performance of the model with respect to separating 1, 2, and 3+ year
...
\begin{enumerate}
 ...
 \item Create uncensored data specific to a chosen assessment time $\tau$,
    ...
    \begin{itemize}
      \item Use the redistribute-to-the-right (RTTR, IPC) algorithm to reassign 
        the 
...
compute the sd of its contribution.  We see below that nodes have by far the
...
followup in the Rotterdam cohort is just over 19 years.
...
the lowest risk group (there are 70 subjects and 0 deaths in the top
2015 curve). 
...
different viewpoint. Image that a local newspaper has predicted the outcome of
...
Now add in the dichotomized measures.  For R-squared, we note that $|R|=0$ for
...
In this case we lack any formal justification for treating the last row below as
...
Both the complimentary log-log and logit have some success with respect to this metric
...
\section{Synthetic estimates}
 Royston and Saurbrei's D, xxx C, ...  They are inappropriate because they 
