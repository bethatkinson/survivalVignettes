...
```{r veteran1b, fig=TRUE, echo=FALSE}
...
```
 ...
 \caption{Solid line = the estimated effect if Karnofsy score, versus time, 
   in the Veteran's cancer data set, as estimated by `cox.zph`, along
...
Each has the appropriate values for the time dependent covariate `tgroup`,
...
create one curve for each line in the input data; the `id`
...
The time scale for the `cox.zph` plot used
...
We can add said line to the `cox.zph` plot.
...
by the p-value for the slope term in `vfit3`.
