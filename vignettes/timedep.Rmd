...
As an example consider a recent analysis from the Mayo Clinic study of aging
...
example based on the advanced lung cancer data set. Assume that subjects came in
...
occur in any given data set. Using such a covariate is similar to jogging across
...
A useful function for building data sets is `tmerge`, which is
...
* $\ldots$: additional arguments that add variables to the data set
...
Where `data1` is the starting data set and additions to the data are
taken from `data2`.
...
The data set is included as `cgd0` in the survival library. Here is the R
...
* Each row of the resulting data set represents a time interval
...
* One can verify that the resulting data set is equivalent to
...
This example is atypical in that we have used a *wide* data set, one with
...
The `jasa` data set contains information from the Stanford heart
...
`pbc2` directly from data set `pbc`.
...
account for those who progress more quickly. In the pbc data set the status
...
for either the time or the value. In the pbcseq data set there are several
observations with a missing alkaline phosphotase value. A consequence of this
behavior is that the pbc2 data set effectively uses "last value carried forward"
...
2453, respectively; in the final data set it is coded 682 from day 1492 until
...
after the last follow-up date of the pbc data set and are ignored. Bilirubin is
...
The data for the pbcseq data set was assembled at a later calendar time than the
primary data set. Since having lab test results is a certain marker that the
...
`pbc` data set. The cutoff rule of `tmerge` is purposefully conservative to
...
Such is not always the case, however. Automatic additions to a data set via
...
identifier, start, and stop variables which are added to the output data set.
...
"progression" get recorded in the data set 1-2 days before the progression
...
Even more subtle biases can occur via coding errors. For any data set containing
...
new variable to the data set. The second line creates a covariate which is a
...
```{r veteran1b, echo=FALSE, fig.cap="Solid line = the estimated effect if Karnofsy score, versus time, in the Veteran's cancer data set, as estimated by `cox.zph`, along with confidence intervals.  Dotted lines are the overall estimate from a Cox model with karno as a time-fixed effect, and a reference line at 0.}
...
creating a new data set with time intervals. We create two hypothetical subjects
...
refine it. In this data set we reasoned that although risk increases with age, a
...
feasible, but for a large data set the first can become very slow, or even
