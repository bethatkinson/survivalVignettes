...
1. The time range of interest. Survival models give predictions over
...
* Software validation, i.e., does a given computer routine produce the
...
will contain ideas on how to *think* about the problem
...
Korn and Simon -@Korn90 and Altman and Royston -@Altman00.
...
In general, the chosen validation metric needs to be *fit for purpose*.
...
* There are at least 3 possible assessments
    ...
    1. The expected number of visits to state $j$ by time $t$, $E(N_j(t))$
    2. The probability of being in state $j$ at time $t$, $p_j(t)$
...
commonly denoted as *discrimnation* and *calibration*.
...
on the eventual *number* of events that will occur, a task that 
...
1. For assessment type 1, the total number of events, we can compare the
...
2. Apply standard censored data methods to the validation data, and compare
...
3. Create uncensored data specific to a chosen assessment time $\tau$,
    then use standard methods for uncensored data. Two approaches are
    
      * Use the redistribute-to-the-right (RTTR) or inverse probability of
...
4. Ignore censoring. A disastrous approach, but one that too often 
...
The `rotterdam` data contains information on 2892 primary breast cancer
patients from the Rotterdam tumor bank.  
The `gbsg` data set contains the patient records of 628 patients with
...
These data sets were introduced by Royston and Altman -@Royston13b
and have been widely used, both are included in the survival package.
@fig-gbsg1 shows overall survival for the two groups.
The Rotterdam study has longer follow-up.



```{r fig-gbsg1, fig.cap="Rotterdam and GBSG data"}
...
```{r fig-gbsg2, fig.cap="Spline fit for number of nodes, Rotterdam"}
...
component. A plot of the estimated shape is shown in @fig-gbsg2. 
...
We can then fit a spline on `tstart`, the time since enrollment.
...
McLernon -@McLernon23 also used the Rotterdam/GBSG pair, but fit their
...
```{r pyears, eval=FALSE, echo=FALSE}
...
and validation data sets have *exactly* the same variables.
...
Muchtar et al -@Muchtar19 used data on 1007 patients with newly diagnosed
...
The `amyloidmodel` data set contains digitized curves from the papers,
and `amyloid` the survival times and per-model risk category for each of
...
## Treatment trial in Primary Biliary Cirrhosis
...
`pbc` data has been part of the survival package for over 2 decades.
...
The `udca` data set contains early results of a trial of ursodeoxycholic 
...
rate.  The `udca` data includes the Mayo risk score, using the coefficients
from the 312 subset.
...
* For $C$ count ties in the response time $t$ as uninformative, 
    such pairs count as neither
    concordant or discordant.  Ties in the predictor $\hat y$ count as 1/2.
* For survival data, pairs such as (5+, 7) are also treated as
...
* The redistribute-to-the-right (RTTR)
...
This approach has been labeled as the "time-dependent AUROC" by @Heagerty00.
...
$$
\begin{align*}
R^2 &= 1 - \frac{\sum_{i=1}^n w_i(\tau) \left[\delta_i - \hat p_i(\tau)\right]^2}
...
$w$ are the IPC weights, $\hat p$ is the predicted probability of death
...
van Houwelingen and Putter -@vanHouwelingen12 show the value of $N$ and
...
Kattan and Gerds -@Kattan18 
redefine $1- N/D$ as the index of predictive accuracy (IPA).

## Breast cancer

@fig-gbsg5b} shows the $C$ statistic with $n(t)$ (Gehan-Wilcoxon or
Harrell) and
...
*and* pairs with both $t_i$ and $t_j$ less than 4 as incomparable pairs.
...
```{r fig-gbsg5b, echo=FALSE}
#| fig-cap: C statistics versus truncation time using predictions from the Rotterdam model, for Rotterdam and GBSG data sets.
...
* For this particular data set pair, the discrimination of the model
...
* Concordance gets worse over time, which is not a surprise.
...
* The $S/G$ weighting gives proportionately larger weights than 
...
* The DC is larger than $C$, but much more erratic with respect the
...
@fig-gbsg5c gives further insight to why $DC > C$.
...
If we focus on the concordance values at $\tau=5$ in @fig-gbsg5b,
the concordance $C$ at that point is an average that omits the upper right
quadrant of @fig-gbsg5c (time 1 and time 2 both $\ge 5$ years) 
while DC omits both the upper right and lower left quadrants
(time 1 $<5$ and time 2 $>5$ or vice versa).



```{r fig-gbsg5c, echo=FALSE}
#| fig-cap: Image plot of the concordance for pairs of observed time values in the Rotterdam data set.  The dashed contour lines are at  .5 (diagonal), .55, .6, .65, .7 and .75.
...
The IJ variance used by the `concordance` function also allows computation
...
The plots do not tell us *which* $\tau$
...
The `reverse = TRUE` argument is needed because a high score is
...
using standard population mortality tables; the `survexp.us` array
...
Berry -@Berry83 shows that $N_i$ has the same likelihood function,
 up to a constant,
...
The use of `subset= (expect >0)` deserves comment.  When there
...
The `glm` function does not know calculus, however, and will generate NA
...
Below are two simple assessments, the first prefigures material from 
@sec-remodel.  In both cases a slope of 1.0 lies well within the
...
```{r fig-gfit2, fig.cap="Calibration plot using observed/expected deaths"} 
...
We can also divide the predicted risk score `eta` into bins to create a
...
* Because the GBSG data set has subjects who are grade 1--3 while the 
...
* A similar check is made for the three size categories. The medium
...
* For nodes, we have broken the distribution into four approximately
...
This is currently a bit clumsy in the software, a `tmax` argument in
the `predict` function, similar to ymax in concordance, would make this
...
$$
  S(t) = (1/n) \sum_{i=1}^n \hat S(t; x_i) 
$$ {#eq-direct}

$$
    \ne \hat S(t; \overline x) 
$$ {#eq-dbad}

where $x$ is the set of covariate vectors for the $n$ validation samples
and $\hat S$ is prediction using the target model.
@eq-direct is called the direct adjusted survival, or in more
recent years as a $g$-estimate.
The curve at the mean covariate, @eq-dbad is not the same.
...
Object `directall` contains 686 separate predicted survivals, 
one per subject.
...
```{r fig-dcox, fig.cap="A check for systematic effect versus the risk score."}
...
Likewise checking for a horizontal line in @fig-dcox is a visual
...
@fig-gfit2, with two differences. First, @fig-dcox is
...
change, not absolute.  Second, the confidence intervals in @fig-gfit2
...
@Austin20, with a couple of changes. 
...
plot(asurv15, col=1:4, lwd=2, xlab="Months", ylab="Survival")
for (i in 0:3) {
    temp <- subset(amyloidmodel, stage==i & study== 2012)
    lines(temp$month, temp$survival, lwd=2, col=i+1, lty=2)
}
legend("topright", paste("Stage", 0:3), lty=1, col=1:4, bty='n')
```
...
* Use the RTTR (or IPW) approach to reassign the case weights of all
...
* Pseudovalues are an alternative to the RTTR which will assign a
    response to censored and non-censored subjects.
* Apply well known  measures for binomial data to the new data, 
...
The overall predicted curve `direct` was computed earlier.
...
independently of the form of the baseline hazard -@Goen05.
