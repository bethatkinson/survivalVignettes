...
language of R along with the fact that the basline hazard can be thought of
...
However, for the 1:5 transtion (0mc:death) the three 0/1 dummy variables
...
and the predicte $\hat y$ for each observation remains unchanged as do all of
...
\subsection*{Summary}
One aspect of this that we have not mentioned is that with a set of transtions 
