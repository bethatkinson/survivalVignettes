...
@ 
  ...
  \label{pbc1}
\end{figure}
PBC is an inflammatory disease, thought to be of
auto-immune origin, characterized by a slow but inexorable loss of the small
...
Further checking shown below shows that 0/4246 female and 4/3675 males who
were ``at risk'' for a 0mc:3mc jump had such an event, the hazard ratio
for male sex \emph{is} infinite.  We will address this issue in
section \ref{sect2}.
...
The underlying code fits all 10 of the transitions in model \code{nfit1}
simultaneously by creating a stacked data sets.
...
the second matrix shows the strata.  In this case, there are separate nafld 
and male coefficients for
each transition, and a separate stratum for each transition.
...
a transition from 0mc to 1mc for the observation.
...
transition (\Sexpr{scount} rows, 2 columns), and etc. to $X^{(10)}$ for the
tenth transition.
...
This matrix contains more values than we need, formally.  Observation 1 for
instance is a row for current state 0mc, for instance, so one could
...
both of them.
We take advantage of the use of ``1'' as the intercept in the formula modeling
language of R along with the fact that the baseline hazard can be thought of
as the ``intercept'' term of a MSH model, for a compact notation.
...
also that 1mc appears to be lower than 0mc.  More on that will be taken
up further below. 
...
coefficients appear but will never be used. For the 2:5 transition only
\code{cstate1mc} is non-zero and similarly for 3:5 and 4:5.
We can get an equivalent model with somewhat simpler printout by creating
...
Another option which provides an even clearer printout is the \code{shared}
option.
...
in a covariate, e.g., replace some covariate $x$ in model with $z= x+2$,
and the predicted $\hat y$ for each observation will remain unchanged,
 as do all of
...
\emph{except} the ph(1:5) coefficients, nor do they shift by a constant.
This means that we must be cautious about interpreting the absolute values
of these coefficients, and perhaps even more any p-values, just as would be
...
predictions.
We point out the following about these predicted survival curves:
\begin{enumerate}
...
The code above recognizes $\gamma$ coefficients via their
special variable name \code{ph(1:5)}. 
(I have little doubt that someone, somewhere will eventually create a strange 
variable whose name is enough like this to fool my code, and then post it 
as a bug.)
...
variable is 
associated with $\gamma$ rather than $\beta$ terms.
...
\code{notused} out of the dummy data set would perhaps be cleaner, but the
\code{model.matrix} routine from base R, which I depend on, is unhappy if a
variable found in the model formula is missing from the data. I don't yet have
a way to work around that.

To do: example using nfit3b, once I have the NA marker debugged.
...
the risk set at time $t$ = (in state A at $t$) + (in state B at $t$) + 
(in state C at $t$) will have no duplicate subjects.
The Cox partial likelihood term for an event at $t$ will have a proper
denominator, i.e., a weighted sum over all those at risk.
...
We have encountered one example, which has motivated the code.  It can be
...
One of the important aspects of this example is that the collection of starting
states are disjoint, i.e., no given starting state appears more than once
(is this true?).  Thus the risk sets are proper.
...
To do: I think that there is a connection to linear fits through the origin,
where a change of covariate has similar consequences.  But we need to work out
whether the analogy is indeed correct, and more importantly whether it will
