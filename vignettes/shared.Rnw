...
#pstate2$pstate <- apply(pstate2$pstate, 1:2, cumsum)
...
of transitions such as A:C and B:C to lead to a common state, disjoint
...
Here is sample code to fit this model, retaining the share coefficients for
mc:mc transition of \code{nfit2}.  Behind the scenes, the code bundles the
...
and the predicted $\hat y$ for each observation remains unchanged as do all of
the non-intercept coefficients and their estimate standard errors.
A much more common case is replacement of a 0/1 covariate 1/0, e.g., one fit
coded 1=male and another 1=female. 
...
predictions.
We point out the following about these fits
\begin{enumerate}
...
The code above recognizes $\gamma$ coefficient by their association with the
special variable name \code{ph(1:5)}. 
(I have little doubt that someone, somewhere will eventually create a strange 
variable enough like this to fool my code, and then post it as a bug.)
...
Identical hazards are a problem child. The problem is that such models can
