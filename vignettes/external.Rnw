...
supperssMessages(library(survey))
...
This overall framework accomdates a large fraction of the methods that have
...
Now, because published validatation data sets are very rare, successful
...
i.e., on the situation specific defintion of successful prediction.
...
who are in extemis often do not enroll in a research study, and some
...
We can then fit a spline on \code{tstart}, the time since enrolloment.
...
Nevetheless, these are the imperfect models we will validate.
...
study, while also getting an estimate of treament effect is simply not feasible.
...
\begin{itemize}
  ...
  \item The $S/G$ weighting gives proportionatly larger weights than 
...
Figure \ref{gbsg5c-1} gives furthur insight to why $DC > C$.
...
the concordance $C$ at that point is an average over all the points in the lower
left quadrant of figure \ref{gbsg5c-1} (time 1 and time 2 both $\le 5$ years) 
while DC is an average over all point in the upper left and lower right
quadrants (time 1 $<5$ and time 2 $>5$ or vice versa).
...
and also plot the extimated standard deviation.
...
We do not recommend the  pseudovalue $C$ at this time, as its behavoir and
...
the observed cumlative hazard, the compensator of the martingale, or
...
thus these observations contriubute 0 to the loglik and can be omitted.
...
Weak calibration, sometimes known as the calibration slope, is an asssessment 
...
\begin{itemize}
  ...
  \item For nodes, we have broken the distribution into four approximatley
...
expected, which implictitly extended all of the model curves to the right.
...
The R squared measures of Nagelkirke, Kent and O'quigley,
...
values $\hat\eta$,
 ...
 on the validation
data, using $\eta\hat_i$ as the predictor for subject $i$, then compute $C_G$
using the rescaled data. This is essentially the same idea as the regression
slope.

\section{Royston's $D$}
The Royston and Saurbrei statistics $D$ and $R_D$ are also based on a particular
...
\end{equation*}
If there are tied $\eta\hat$ values replace those $z$ values with their average.
...
Last, compute the three estimates.  (Since $z$ has standard devation 1, 
...
is easy, i.e., to form reference $\eta\hat^*$ using the prior coefficients and
the validiation data. But a z-transform of these new values will not be the same
...
Nalgelkirke, Kent and O'Quigley; yet to do.
