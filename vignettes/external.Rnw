...
legend(10, .4, c("GBSG", "Rotterdam node positive", "Rotterdam node negtive"),
       lty=1:3, lwd=2, bty='n')
@
\caption{Rotterdam and GBSG data}
\label{gbsg1}
\end{figure}
...
abline(v=9, col="gray")
@
\caption{Spline fit for number of nodes, Rotterdam}
\label{gbsg2}
\end{figure}
...
@ 
\caption{C statistics versus truncation time using predictions from the Rotterdam model, for Rotterdam and GBSG data sets.}
\label{gbsg5b}
\end{figure}
...
abline(0,1, lty=2)
@ 
\caption{Image plot of the concordance for pairs of observed time values in the Rotterdam data set.  The dashed contour lines are at  .5 (diagonal), .55, .6, .65, .7 and .75.}
\label{gbsg5c}
\end{figure}
...
@
\caption{Calibration plot using observed/expected deaths}
\label{gfit2}
\end{figure}
...
@
\caption{A check for systematic effect versus the risk score.}
\label{gcox}
\end{figure}
