...
@fig-T2 (T2) shows the two Kaplan-Meier curves, with one going uphill and the
...
@fig-T3 (T3) shows the Aalen-Johansen curves in the same form. The
...
@fig-T4 (T4) reorders the states so the event free is the top
group. This author prefers the unstacked version (@fig-putter1), which
...
The last @fig-T9 (T9) in this section of the tutorial contains
...
Now draw @fig-T14 (T14) for baseline hazards. 
...
@fig-T15 (T15) separates the remission/death state into two
...
lines.  @fig-T15 (T15) corresponds to the left panel.
