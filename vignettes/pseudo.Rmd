---
title: "Pseudovalues for survival data"
...
output: 
    bookdown::html_document2:
        ...
        toc: true
vignette: |
  %\VignetteIndexEntry{Pseudovalues for survival data}
...
For any positive probability distribution, a well known identity is that
...
$\tau$ years since initiation.  If $\tau=10$ and the area under the curve
...
i.e. everyone has to be somewhere.
...
RMST and its standard error directly from the print routine for `survfit`. 
...
An observation censored before the first event has residual 0, so its pseudo
value has no variance at all.
In such a case theory argues for using White's variance estimate for the linear
model fit, which accounts for possible heteroscedasticity.
...
pseudo value per subject.  
When there are pseudo values at multiple reporting times, and thus multiple
observations per subject, then the correction becomes essential.
Deficiencies in R's `geese` function lead us to use the survey
...
standard extraction functions such as `coef` and `vcov` have not
been implemented.
The `survey` package requires that the design be specified in advance;
...
For example, the `lung` data set comes from a multi-center study, and
contains an identifier for the institution from which the subject was recruited.
We might want to adjust for the possibility that different institutions recruit
...
Both the matrix of IJ values from `resid` and the matrix of 
IJ pseudovalues from the `pseudo` function
...
The `data.frame` argument causes the values to be returned in long form
as a data.frame.
...
robust sandwich estimator either via survey sampling or a GEE argument.
These are shown below.  The `geese` function has the unfortunate feature
...
are clustered around 0 and 1, mimicking binomial data.
...
question, fails directly.  First, it does not accommodate response values
...
`cpdat\$pdeath` fails both of these.
...
per subject, but in the case of heteroscedasticity, however.
...
The same out of range error occurs with the simple `glm` function.
...
in the documentation for `family`, or on examination of the glm
`make.link` function.  We call the resulting function `blogit` for
...
The pseudo-value matrix has 3 columns corresponding to each of the 3 states,
