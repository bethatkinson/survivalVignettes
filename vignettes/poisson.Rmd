---
title: Poisson models and person years
...
form.  Here is a table by sex and age for the mgus data set:
...
temp <- glm(pfit1$event[1,1] ~ 1+ offset(log(pfit1$pyears[1,1])), poisson)
...
follow-up time apppears as a covariate with a known coefficient of 1.
...
# etime = time since erollment, year = year of enrollment
...
In some cases we may want to use the center of a cagegory as the numeric value.
...
pre to post transplant, for those who recieved a transplant;
...
The United States death rates are included in the survial package, obtained from
