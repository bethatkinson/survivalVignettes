...
This approach has been labeled as the ``time-dependent AUROC'' by 
\cite{Heagerty05}.
...
\subsection{Breast cancer}
Figure \ref{fig:gbsg5b} shows the $C$ statistic with $n(t)$ (Gehan-Wilcoxon or
Harrell) and
...
Figure \ref{gbsg5c} gives furthur insight to why $DC > C$.
...
If we focus on the concordance values at $\tau=5$ in figure \ref{gbsg5b},
the concordance $C$ at that point is an average that omits the upper right
quadrant of figure \ref{gbsg5c} (time 1 and time 2 both $\ge 5$ years) 
...
\cite{Austin20}, with a couple of changes. 
