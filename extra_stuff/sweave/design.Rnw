...
%\VignetteIndexEntry{Design of the Survival Libraries}
...
and I still occasionally interface with it.
...
embedded into a system with rich tools for data manipulation and
presentation.
...
for all of these extending the time to event (progression, re-transplant, death,
...
design goals have remained fairly constant.
\begin{enumerate}
  ...
  \item Flexibility.  I often mix and match approaches from prior functions, 
    or use their results in new ways that were unforeseen at the time of writing. 
  ...
  \item Reliability.  One of my colleagues said this best during the interview of
...
the usual print function with \code{quote=FALSE}.
...
\begin{itemize}
  ...
  \item plotting methods: print a ``not valid'' error message for barplot, 
  ...
  \item mean, median, quantile: we chose to leave the mean undefined (print
  ...
  \item c: concatenate Surv objects.  This turns out to be harder than one 
...
print
@ 
Thus the \code{print.coxph} function also should have exactly two arguments,
...
(This design mistake will be with me forever, to support backwards compatibility
for old code.)
...
The basic trick is to find out what the variables are, create a temporary
...
form that occurred in early versions of Splus and R.  That has
...
That is, it is a completely legal model, but not something I anticipated,
...
used to decide the computational approach.  
...
and contrast information is returned as an attribute of the $X$ matrix.
...
\end{verbatim}
By using the terms from the original fit as the ``formula'' argument
...
frame, and a later computational step needs more data information than
...
coxme function makes heavy use of a library for 
...
This is attainable for the simple structures found in a CS classroom, a date, or
even a matrix.  
...
\emph{can} be done as well being an an illustration of how difficult the task
...
to exist for backwards comparability, but it should not be used for
any new code.  
...
(both well used),
both frequentest and Bayes methods in my analyses, 
...
``what to keep''?
There is a trade-off between precomputing and keeping everything that a
...
\begin{itemize}
      ...
      \item Martingale residuals and linear predictor (as in lm).  
        At the time I
        wrote coxph martingale residuals were the most often used, 
        enough so that
       ...
       \item A vector of means, which was used to center the covariates 
        ...
        more stable if
        you do it.  Also, it has been traditional to report the baseline 
...
Since that time the following portions have been created, listed
...
A prediction that contains standard errors requires more data than is
...
Oncology Group (ECOG) definition of performance status.
...
For instance consider the first of these which has only six observations
...
The maximum likelihood solution using the Breslow approximation is
$r = (1/2)(3 + \sqrt{33})$.
...
notebook with the penciled derivations as a memento), but has
paid for itself a dozen times over. 
When validating a calculation I know not just the final answer but 
...
\begin{itemize}
  ...
  \item Printouts.  For a few moderate size data sets, 
    ...
    I have not created an equality check, and mistakes in the print functions
...
All user-visible R functions in a library should have a help file.  A
skeleton is easily created using the \code{prompt} function.  The
R documentation has a thorough description of what information they should
...
for users who already know about your functions and have forgotten a few
details.
...
\section{Using noweb}
\subsection{Code}
A last area of documentation is of the code itself.
...
maintenance using the literate programming model introduced by
D Knuth.
\begin{quotation}
...
Noweb files look almost identical to Sweave files; this should be
no surprise since Sweave was based on noweb.
Most of my noweb code uses a \emph{.Rnw} suffix, taking
advantage of the very capable emacs modes that are part of the ESS package.
...
noweb program using the standard system installation tools.
...
\begin{enumerate}
  ...
  \item The formatting of the output is better tailored to code 
    presentation.
  \item The use of unordered subordinate clauses is fully supported.
\end{enumerate}
...
``currently at risk''.
This calculation is not usually amenable to vectorizing, meaning that
...
some time after the C routine (and the R function that called it) have finished
leading to a search for ``bugs'' in another function that has none, mistaken
accusations of a problem in base R itself, etc.
...
of the computation.
R has also gotten more efficient: it used to make a copy of 
...
Here is a moderately complex example motivated by a multi-institutional
...
parenthesized expression like a call with a single argument.
...
by definition this is a parenthesized term whose first child in the
...
otherwise known as \emph{frailty} models, I
...
\begin{itemize}
  ...
  \item I followed the same path with survexp.  In this case the user view
    ...
    rate table argument.  The resulting survexp object, however, has to
...
Here is a copy of the relevant portion from the frailty.gamma
function.
\begin{verbatim}
...
of the function).  This includes all of the local variables plus a pointer
...
There is a corollary to this that arises in ordinary work.  Consider the
following code:
\begin{verbatim}
