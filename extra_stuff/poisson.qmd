...
@tbl-table1 is an example, and shows how a subject may contribute to
...
Table: ({#tbl-table1}) Person-years analysis for one person who entered the study at
...
cuttime <- tcut(rep(0, nrow(spe)), c(0:23 *30.5, 365.25*2:10, 36525),
                labels=c(paste(0:23, '-', 1:24, ' mon', sep=''),
                         paste(2:9, '-', 3:10, ' yr', sep=''), '10+ yr'))
cutage <- tcut(spe$age, 365.25*c(0,40:85,110),
               labels= c("<40", paste(40:84, '-', 41:85, sep=''), "85+"))
spe$testdt <- arsenal::mdy.Date(7,1,spe$testyr)
## Save the dataset for further analysis
tmpfit <- pyears(Surv(futime, death) ~ cuttime + cutage + sex
                 + mgus + ratetable(age=age, sex=sex, year=testdt),
                 data=spe, ratetable=survexp.mn, data.frame=T)
...
```{r }
#| fig-cap: The estimated selection effect for female patients with negative SPE,
...
## Defining the range from both pyrs.spe and spe allows us to obtain
## predictions 
...
age.range <- range(c(pyrs.spe$age, spe$age/365.25)) 
...
AgeWeights <- table(cut(spe$age/365, breaks=seq(20,105,5), left.include=T))/N
